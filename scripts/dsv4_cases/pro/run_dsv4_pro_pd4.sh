...
LOG_DIR=${LOG_DIR:-/home/t00937989/scripts/logs_dsv4_pro_pd4}
...
setup_runtime_env() {
    ...
    source_ascend_env /usr/local/Ascend/ascend-toolkit/latest/opp/vendors/pro_transformer/bin/set_env.bash
...
start_prefill() {
    ...
    echo "[prefill] ip=${ip} rank=${rank}/${PREFILL_NNODES} tp=${PREFILL_TP} dp=${PREFILL_DP} max_running=${MAX_RUNNING_REQUESTS} graph='${CUDA_GRAPH_BS}' overlap=${PREFILL_ENABLE_OVERLAP_SCHEDULE} log=${log}"
    python3 -m sglang.launch_server \
        ...
        --dp-size "${PREFILL_DP}" --enable-dp-attention \
        --moe-a2a-backend deepep --deepep-mode auto \
        --quantization modelslim --enable-dp-lm-head \
        --kv-cache-dtype auto \
        --skip-server-warmup \
        --cuda-graph-bs "${graph_bs[@]}" \
...
start_decode() {
    ...
    read -r -a graph_bs <<< "${CUDA_GRAPH_BS}"
    echo "[decode] ip=${ip} rank=${rank}/${DECODE_NNODES} tp=${DECODE_TP} dp=${DECODE_DP} max_running=${MAX_RUNNING_REQUESTS} graph='${CUDA_GRAPH_BS}' overlap=${DECODE_ENABLE_OVERLAP_SCHEDULE} log=${log}"
    python3 -m sglang.launch_server \
        --model-path "${MODEL_PATH}" \
        --page-size 128 \
        --tp-size "${DECODE_TP}" \
        ...
        --dp-size "${DECODE_DP}" --enable-dp-attention \
        --moe-a2a-backend deepep --deepep-mode auto \
        --quantization modelslim --enable-dp-lm-head \
        --kv-cache-dtype auto \
        --skip-server-warmup \
